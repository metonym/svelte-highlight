const ESQL_KEYWORDS =
  "FROM ROW SHOW WHERE EVAL STATS INLINE KEEP DROP RENAME SORT LIMIT DISSECT GROK ENRICH " +
  "MV_EXPAND LOOKUP JOIN ON BY AS DESC ASC AND OR NOT IN LIKE RLIKE IS " +
  "NULL WITH METADATA CHANGE_POINT FORK TS";

const ESQL_FUNCTIONS =
  "COUNT COUNT_DISTINCT AVG SUM MIN MAX MEDIAN QSTR MATCH KQL DATE_TRUNC DATE_PARSE " +
...
function defineEsql(hljs) {
  ...
  return {
    ...
    contains: [COMMENT, STRING, PIPE, CAST, FIELD, NUMBER],
  };
}
