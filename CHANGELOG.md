...
- step incremental parses through a loaded document
- scan `splitLines` by index instead of per character
...
- drop Zig `usingnamespace`, `async`, and `await` keywords
- recognize Nushell `export-env` and `hide-env`
- convert Mathematica's builtin-symbol word-list guard to IR
- convert G-code's letter-boundary callback to IR

## [7.18.1](https://github.com/metonym/svelte-highlight/releases/tag/v7.18.1) - 2026-08-24

**Fixes**

- zero runtime dependencies (dropped `magic-string`)
...
- add `virtualize` prop to `HighlightStream` for windowed rendering
- upgrade `highlight.js` to version [11.12.0](https://github.com/highlightjs/highlight.js/releases/tag/11.12.0)
- add custom `cql` language
- add custom `flux` language
- add custom `prql` language

## [7.16.2](https://github.com/metonym/svelte-highlight/releases/tag/v7.16.2) - 2026-07-30

**Performance**

- checkpoint incremental tokenization every 32 lines instead of every line
- append sealed `HighlightStream` chunks without an O(c) spread copy
- avoid rebuilding completed `HighlightStream` HTML every frame
- O(delta) DOM paint for pure appends in `HighlightEditable`

## [7.16.1](https://github.com/metonym/svelte-highlight/releases/tag/v7.16.1) - 2026-07-27

**Performance**

- make the `Typewriter` fallback split incremental
- read `HighlightEditable` caret offsets from line lengths
- cache reopen tags across line breaks in `extendLines`
- binary-search ranges for single-line `HighlightEditable` repaints
- resolve preprocessor match line numbers lazily
- memoize missing closing tags in the JSX guard
- use a `Set` for `LineNumbers` highlighted-line lookups
- defer guard closure allocation to cache misses
- repaint `css-highlights` in one linear sweep
...
- add custom `vue` language
- add custom `mdx` language
- add custom `marko` language

**Fixes**

- inline theme background images in generated `highlight.js` styles

## [7.10.1](https://github.com/metonym/svelte-highlight/releases/tag/v7.10.1) - 2026-06-02

**Fixes**

- fix custom `svelte` grammar for TypeScript in script blocks and markup

## [7.10.0](https://github.com/metonym/svelte-highlight/releases/tag/v7.10.0) - 2026-06-02

**Features**

- add custom `html` language
- add custom `svelte` language
- add custom `astro` language

**Fixes**

- support `langtag` usage with `LineNumbers`
...
- upgrade `highlight.js` to version [11.10.0](https://github.com/highlightjs/highlight.js/releases/tag/11.10.0)
...
## [7.4.4](https://github.com/metonym/svelte-highlight/releases/tag/v7.4.4) - 2023-12-16
